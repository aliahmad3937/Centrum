...
object SavedPreference {
    ...
    fun getUserName2(): String{
        val value:UserResponse.Data? = getUserData(application!!)
        return if (value == null) "User name" else "${value.firstname} ${value.lastname}"
    }
    ...
    fun setUserData(context: Context, data: UserResponse.Data) {
    ...
    fun getUserData(context: Context): UserResponse.Data? {
        //We read JSON String which was saved.
        val value = getSharedPreference(context)?.getString(USERDATA, null)
        return if (value == null) null else GsonBuilder().create().fromJson(value, UserResponse.Data::class.java)
    }
